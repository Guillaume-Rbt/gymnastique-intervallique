...
export default function Header({ score, onScoreChange, running, resetSignal, paused, ref, }: HeaderProps) {
    ...
    const { progress, needCommit, commitAllowedIntervals, gameState, allowedIntervals } = useGameContext();
    ...
    const [SettingsPopup, setIsOpenTrue] = usePopup(false, onPopupClose);


    useImperativeHandle(ref, () => ({ setIsOpenTrue }), [])
    ...
    return <div className="text-3 header position-fixed xs:flex-justify-center gap-15 sm:gap-10 xs:gap-7 sm:px-10 z-10 color-dark-800 w-full top-0 flex-items-center flex-justify-start flex bg-slate-100 h-16 ">
        ...
        <Button onClick={setIsOpenTrue} classes="p-[3px] text-8 bg-transparent hover:bg-gray-200 transition-bg duration-200 rounded-full"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
        ...
        {createPortal(<GameSettingsPopup allowedIntervalsRef={allowedIntervalsRef} Popup={SettingsPopup} />, document.querySelector("#root") as HTMLElement)}
    </div >
}